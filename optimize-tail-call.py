...
if len(sys.argv) == 0:
    print('optimize-tail-calls needs a .py file to run')
    exit(1)

with open(sys.argv[1]) as f: module_text = f.read()
...
def replace_tail_calls(function_code, tail_call_line_locations):
    ...
    for future_instr in dis.get_instructions(function_code):
        ...
        if instr.opname == 'CALL_FUNCTION' and future_instr.opname == 'RETURN_VALUE' and current_line in tail_call_line_locations:
            ...
            # we use a footer so we don't destroy the indexes in other parts of the function
            # note we could really just have ONE footer b/c we know all the function calls are the same
            # TODO this could improve code size
            footer_location = len(payload)

            # yes I know this concat is n^2 maybe I'll fix this later
            payload = payload[0:pos] + pack_op('JUMP_ABSOLUTE', footer_location) + payload[pos+2:]
            for arg_n in reversed(range(nargs)):
                payload += pack_op('STORE_FAST', arg_n)

            # we pop the function handle off the top of the stack
            payload += pack_op('POP_TOP', 0)

            # in this case 0 jumps to the beginning of the function
            payload += pack_op('JUMP_ABSOLUTE', 0)
        instr = future_instr  
...
# uncomment below if you want to see the new vs old bytecode
#dis.dis(module_code)
#dis.dis(new_code)
